...
process.on("uncaughtException", (err) => {
  console.log("Nicht abgefangene Ausnahme: " + err);

  const exceptionembed = new MessageEmbed()
  .setTitle("Nicht abgefangene Ausnahme")
  .setDescription(`${err}`)
  .setColor("RED")
...
process.on("unhandledRejection", (reason, promise) => {
  console.log(
    "[FATAL] Möglicherweise unveränderte Ablehnung bei: Promise ",
    promise,
    " Grund: ",
    reason.message
  );
  ...
  .setTitle("Ich habe einen Fehler gefunden!! Bitte behebe diesen Fehler.")
...
client.login(BOT_TOKEN).then(() => {
  console.log(
    chalk.bgBlueBright.black(
      ` Erfolgreich eingeloggt als: ${client.user.username}#${client.user.discriminator} ✅ `
    )
  );
});
